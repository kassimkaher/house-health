...
# Multi-stage build for apps/worker. Same layout as api.Dockerfile — see
# that file's comments for the rationale.
...
RUN pnpm --filter @hh/worker... build
...
COPY packages/pipeline/package.json packages/pipeline/package.json
COPY packages/config/package.json packages/config/package.json
RUN pnpm install --frozen-lockfile --prod --filter @hh/worker...
...
# repo-root node_modules. Each shared package's node_modules must be copied
# individually or it can't resolve its own dependencies at runtime (e.g.
# packages/config -> zod). packages/notifications has no runtime deps of
# its own, so it has no node_modules to copy.
...
COPY --from=prod-deps /repo/apps/worker/node_modules/@nestjs ./packages/contracts/node_modules/@nestjs
...
# package's own node_modules, so copy them from apps/worker's (which has
# them as real dependencies) or Node's directory-walk resolution fails.
COPY --from=prod-deps /repo/apps/worker/node_modules/@nestjs ./packages/auth/node_modules/@nestjs
